import { formatStr, parseTime } from '@/utils/index'
...
const print = {
  ...
  order(order) { // 订单处理信息
    var contents = Store.store.settings.printer.template.split('\n')
  ...
  accounts(orderInfo, username, valid = false) { // 结账打印
    return new Promise(async(resolve, reject) => {
      this.contents = []
      await this.accountsHandler(orderInfo)
      escpos.print(this.contents, valid).then(response => {
        resolve(response)
      }).catch(err => {
        reject(err)
  ...
  async accountsHandler(orderInfo, username) { // 订单处理信息
    const settings = Store.store.settings
    var contents = settings.printer.accountsTemplate.split('\n')
    ...
    orderInfo.pays.forEach(element => {
      if (settings.isTotal || element.payId > 0) { // 不显示现金
        pays.push({
          type: element.type,
          name: element.name,
    ...
    contents = contents.map(element => {
      ...
      element = element.replace(/{{\s*userId}\s*}/g, username)
